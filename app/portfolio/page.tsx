...
export default async function PortfolioPage() {
    ...
    if (nepalRates && nepalRates.length > 0) {
        // Find Tola prices
        const goldRate = nepalRates.find((r: any) => r.key === "fine-gold-tola")?.price ||
            nepalRates.find((r: any) => r.name === "Fine Gold" && r.unit === "Tola")?.price || 0;

        const silverRate = nepalRates.find((r: any) => r.key === "silver-tola")?.price ||
            nepalRates.find((r: any) => r.name === "Silver" && r.unit === "Tola")?.price || 0;
